...
class GameDisplay(threading.Thread):
    ...
    def drawMapGeneratorWFC(self):
        ...
        # map information
        sizeX = len(self.CO.mapController.getCurrentMap(False).myMap)
        sizeY = len(self.CO.mapController.getCurrentMap(False).myMap[0])
        sizeXOneSquare = 1600 / sizeX
        sizeYOneSquare = 900 / sizeY
        startX = self.CO.mapController.getCurrentMap(False).playerStartX
        startY = self.CO.mapController.getCurrentMap(False).playerStartY
        countX = 0
        countY = 0
        while startX >= 0:
            startX -= sizeXOneSquare
            countX += 1
        while startY >= 0:
            startY -= sizeYOneSquare
            countY += 1
        playerStartX = str(countX)
        playerStartY = str(countY)
        ...
        settingsData = [str(self.CO.mapController.mapGeneratorX), str(self.CO.mapController.mapGeneratorY), "", "", playerStartX, playerStartY, playerStartDirection]
        ...
        for i in range(len(settingsText)):
            ...
            if i+4 < len(self.CO.generateMapButtons):
        ...
        self.drawMap(self.CO.mapController.getCurrentMap(self.CO.officialMaps).myMap, topLeft, bottomRight, (countX-1, countY-1))

        # border
        pygame.draw.line(self.screen, (0, 0, 0), topLeft, (bottomRight[0], topLeft[1]), int(newTextSize / 15))  # top
        pygame.draw.line(self.screen, (0, 0, 0), (bottomRight[0], topLeft[1]), bottomRight,
                         int(newTextSize / 15))  # right
        pygame.draw.line(self.screen, (0, 0, 0), bottomRight, (topLeft[0], bottomRight[1]),
                         int(newTextSize / 15))  # bottom
    ...
    def drawMap(self, myMap=None, topLeft=None, bottomRight=None, highlight=None):
        if myMap is None:
                ...
                for j in range(len(myMap[i])):
                    if myMap[i][j] != -1:
                        ...
                        if highlight is not None:
                            if i == highlight[0] and j == highlight[1]:
                                highlighter = pygame.Surface(((mapWidth / len(myMap)) + 1, (mapHeight / len(myMap[i])) + 1))
                                highlighter.fill((70, 70, 70))
                                self.screen.blit(highlighter, ((mapWidth / len(myMap) * i) + topLeft[0], (mapHeight / len(myMap[i]) * j) + topLeft[1]), special_flags=pygame.BLEND_RGBA_ADD)
        else:
    ...
    def drawPlayerRays(self):
        ...
        for player in self.CO.players:
            if (self.CO.currentMode == "singleplayer" and i == 0) or self.CO.currentMode == "multiplayer":
                ...
                pygame.draw.line(self.screen, (100, 255, 255), ((player.x * self.windowWidth) / 1600, (player.y * self.windowHeight) / 900), (((player.x * self.windowWidth) / 1600) + ((player.nextCheckpointLength * self.windowWidth) / 1600) * np.cos(np.deg2rad(player.nextCheckpointDirection)), ((player.y * self.windowHeight) / 900) + ((player.nextCheckpointLength * self.windowHeight) / 900) * np.sin(np.deg2rad(player.nextCheckpointDirection))), 4)
            i += 1
    ...
    def drawBotRays(self):
        for bot in self.CO.bots:
            for ray in bot.player.frontRays + bot.player.backRays:
                ...
                y2 = y1 + rayLengthY * np.sin(np.deg2rad(ray.direction))
                pygame.draw.line(self.screen, color, (x1, y1), (x2, y2), 4)
                pygame.draw.line(self.screen, (100, 255, 255),((bot.player.x * self.windowWidth) / 1600, (bot.player.y * self.windowHeight) / 900), (((bot.player.x * self.windowWidth) / 1600) + ((bot.player.nextCheckpointLength * self.windowWidth) / 1600) * np.cos(np.deg2rad(bot.player.nextCheckpointDirection)), ((bot.player.y * self.windowHeight) / 900) + ((bot.player.nextCheckpointLength * self.windowHeight) / 900)  * np.sin(np.deg2rad(bot.player.nextCheckpointDirection))), 4)
